...
def help_m1_approval_chain(env="Development", apps={"student": ["ADM", "FIN"], "financial": ["PRO", "RFE"]},
                           res={"student": "this is a reason to request access",
                                "financial": "this is a reason to request access"}):
    mf = init()
    mf.navigate_to_form()
    mf.enter_manager("Scott Gibson")
...
def test_m1_approval_prd_adm():
    chain = help_m1_approval_chain(env="Production", apps={"student": ["ADM"]},
                                   res={"student": "this is a reason to request access"})
    st = set(chain)
...
def test_m1_approval_prd_fin():
    chain = help_m1_approval_chain(env="Production", apps={"student": ["FIN"]},
                                   res={"student": "this is a reason to request access"})
    st = set(chain)
