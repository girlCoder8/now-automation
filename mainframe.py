...
class Mainframe(ServiceNow):
    log_id = "Mainframe"
    form_url = 'https://dev.service-now.com/itsupport?id=sc_cat_item&sys_id=9197ec3edbdc8410965bd5ab5e961963'
    req_url = 'https://dev.service-now.com/itsupport?id=my_requests'
    app_url = 'https://dev.service-now.com/itsupport?id=approvals'
    temp_app_url = 'https://dev.service-now.com/hosting?id=approvals'
    ...
    def select_environment(self, env):
        ...
        self.driver.find_element(By.CSS_SELECTOR,
                                 "#sp_formfield_environments > div > label:nth-child(%d) > input" % self.envs[
                                     env.lower()]).click()
    ...
    def select_specific_dataset(self, pre, access, ds, res=""):
        ...
        self.driver.find_element(By.CSS_SELECTOR,
                                 "#sp_formfield_dataset_access_type > label:nth-child(%d) > span" % self.accs[
                                     access.lower()]).click()
    ...
    def get_dataset_help(self):
        return self.driver.find_element(By.CSS_SELECTOR,
                                        "#specific_datasets_instructions > div > span > div > section").text
    ...
    def get_chain(self, app, typ):
        ...
        time.sleep(self.expl_wait)
        self.driver.find_element(By.CSS_SELECTOR,
                                 "#gsft_nav > div > magellan-favorites-list > ul > li.sn-widget.ng-scope.selected > div > div:nth-child(1) > a > div:nth-child(2) > span").click()
        time.sleep(self.expl_wait)

        self.log("Locating Row")
        self.driver.execute_script(
            "document.getElementById(\"u_access_management_crosswalk_filter_toggle_image\").click()")
